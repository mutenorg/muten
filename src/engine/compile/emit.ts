...
export const RUNTIME = `let __current = null;
  ...
  function __chart(svg, rows, enc, legend) { rows = Array.isArray(rows) ? rows : []; const NS = 'http://www.w3.org/2000/svg'; const cs = getComputedStyle(svg); const num = (n, d) => { const v = parseFloat(cs.getPropertyValue(n)); return isFinite(v) ? v : d; }; const W = svg.clientWidth || num('--chart-w', 320), H = svg.clientHeight || num('--chart-h', 200), pL = num('--chart-pad-left', 34), pR = num('--chart-pad-right', 12), pT = num('--chart-pad-top', 12), pB = num('--chart-pad-bottom', 24), iw = W - pL - pR, ih = H - pT - pB, TICKS = Math.max(1, Math.round(num('--chart-ticks', 4))), BARGAP = num('--chart-bar-gap', 0.42), DONUT = num('--chart-donut-inner', 0.62); svg.setAttribute('viewBox', '0 0 ' + W + ' ' + H); const mk = (t, a, tx) => { const e = document.createElementNS(NS, t); for (const k in a) e.setAttribute(k, a[k]); if (tx != null) e.textContent = tx; return e; }; while (svg.firstChild) svg.removeChild(svg.firstChild); if (!rows.length) return; const fmtN = (v) => { const a = Math.abs(v), r = (x) => { const s = x.toFixed(1); return s.slice(-2) === '.0' ? s.slice(0, -2) : s; }; return a >= 1e9 ? r(v / 1e9) + 'B' : a >= 1e6 ? r(v / 1e6) + 'M' : a >= 1e3 ? r(v / 1e3) + 'k' : String(Math.round(v)); }; const kind = enc.kind || 'bar', radial = kind === 'pie' || kind === 'donut'; const cf = enc.color || (radial ? enc.x : ''), six = {}; let sn = 0; const scls = (b, r) => { if (!cf) return b; const k = String(r[cf]); return b + ' mu-chart-s' + ((k in six) ? six[k] : (six[k] = sn++)); }; const doLegend = () => { if (!legend) return; while (legend.firstChild) legend.removeChild(legend.firstChild); if (cf) for (const k in six) { const li = document.createElement('li'); li.className = 'mu-chart-legend-item'; const sw = document.createElement('span'); sw.className = 'mu-chart-swatch mu-chart-s' + six[k]; const tx = document.createElement('span'); tx.className = 'mu-chart-legend-label'; tx.textContent = k; li.appendChild(sw); li.appendChild(tx); legend.appendChild(li); } }; if (radial) { const arc = (X, Y, R, a0, a1, RI) => { const rad = (d) => (d - 90) * Math.PI / 180, s = rad(a0), e = rad(a1), lg = (a1 - a0) > 180 ? 1 : 0; const x0 = X + R * Math.cos(s), y0 = Y + R * Math.sin(s), x1 = X + R * Math.cos(e), y1 = Y + R * Math.sin(e); if (RI <= 0) return 'M' + X + ',' + Y + ' L' + x0 + ',' + y0 + ' A' + R + ',' + R + ' 0 ' + lg + ' 1 ' + x1 + ',' + y1 + ' Z'; const xi0 = X + RI * Math.cos(e), yi0 = Y + RI * Math.sin(e), xi1 = X + RI * Math.cos(s), yi1 = Y + RI * Math.sin(s); return 'M' + x0 + ',' + y0 + ' A' + R + ',' + R + ' 0 ' + lg + ' 1 ' + x1 + ',' + y1 + ' L' + xi0 + ',' + yi0 + ' A' + RI + ',' + RI + ' 0 ' + lg + ' 0 ' + xi1 + ',' + yi1 + ' Z'; }; const total = rows.reduce((a, r) => a + (Number(r[enc.y]) || 0), 0) || 1; const RAD = Math.min(iw, ih) / 2 - 2, RI = kind === 'donut' ? RAD * DONUT : 0, ccx = pL + iw / 2, ccy = pT + ih / 2; let a = 0; for (const r of rows) { const v = Number(r[enc.y]) || 0, a1 = a + v / total * 360; svg.appendChild(mk('path', { d: arc(ccx, ccy, RAD, a, a1, RI), class: scls('mu-chart-slice', r) })); a = a1; } if (kind === 'donut') svg.appendChild(mk('text', { x: ccx, y: ccy, class: 'mu-chart-total', 'text-anchor': 'middle', 'dominant-baseline': 'central' }, fmtN(total))); doLegend(); return; } const rawMax = Math.max(0, ...rows.map((r) => Number(r[enc.y]) || 0)); let step = 1, mx = 1; if (rawMax > 0) { const rough = rawMax / TICKS, mag = Math.pow(10, Math.floor(Math.log10(rough))), norm = rough / mag; step = (norm <= 1 ? 1 : norm <= 2 ? 2 : norm <= 2.5 ? 2.5 : norm <= 5 ? 5 : 10) * mag; mx = Math.ceil(rawMax / step) * step; } const sy = (v) => pT + ih - (v / mx) * ih; for (let v = 0; v <= mx + step / 1e6; v += step) { const y = sy(v); svg.appendChild(mk('line', { x1: pL, y1: y, x2: W - pR, y2: y, class: 'mu-chart-grid' })); svg.appendChild(mk('text', { x: pL - 7, y: y + 3, class: 'mu-chart-tick', 'text-anchor': 'end' }, fmtN(v))); } const n = rows.length, bw = iw / n; if (kind === 'scatter') { const xs = rows.map((r) => Number(r[enc.x]) || 0), xmax = Math.max(1, ...xs), xmin = Math.min(0, ...xs), xr = (xmax - xmin) || 1; rows.forEach((r) => svg.appendChild(mk('circle', { cx: pL + ((Number(r[enc.x]) || 0) - xmin) / xr * iw, cy: sy(Number(r[enc.y]) || 0), r: 4, class: scls('mu-chart-dot', r) }))); } else if (kind === 'line' || kind === 'area' || kind === 'point') { const pts = rows.map((r, i) => [pL + bw * (i + 0.5), sy(Number(r[enc.y]) || 0)]); const smooth = (ps, k) => { if (ps.length < 2) return ps.length ? 'M' + ps[0][0] + ',' + ps[0][1] : ''; let d = 'M' + ps[0][0] + ',' + ps[0][1]; for (let i = 0; i < ps.length - 1; i++) { const p0 = ps[i - 1] || ps[i], p1 = ps[i], p2 = ps[i + 1], p3 = ps[i + 2] || p2; d += ' C' + (p1[0] + (p2[0] - p0[0]) * k) + ',' + (p1[1] + (p2[1] - p0[1]) * k) + ' ' + (p2[0] - (p3[0] - p1[0]) * k) + ',' + (p2[1] - (p3[1] - p1[1]) * k) + ' ' + p2[0] + ',' + p2[1]; } return d; }; const line = smooth(pts, 0.18); if (kind === 'area') { const gid = 'mcg' + Math.floor(Math.random() * 1e9); const df = mk('defs', {}), lg = mk('linearGradient', { id: gid, x1: 0, y1: 0, x2: 0, y2: 1 }); lg.appendChild(mk('stop', { offset: '0%', 'stop-color': 'currentColor', 'stop-opacity': 0.28 })); lg.appendChild(mk('stop', { offset: '100%', 'stop-color': 'currentColor', 'stop-opacity': 0 })); df.appendChild(lg); svg.appendChild(df); const fill = mk('path', { d: line + ' L' + pts[n - 1][0] + ',' + (pT + ih) + ' L' + pts[0][0] + ',' + (pT + ih) + ' Z', class: 'mu-chart-area' }); fill.setAttribute('style', 'fill:url(#' + gid + ');fill-opacity:1'); svg.appendChild(fill); } if (kind !== 'point') svg.appendChild(mk('path', { d: line, class: 'mu-chart-line', fill: 'none', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' })); if (kind !== 'area') rows.forEach((r, i) => svg.appendChild(mk('circle', { cx: pts[i][0], cy: pts[i][1], r: 3, class: scls('mu-chart-dot', r) }))); } else { rows.forEach((r, i) => { const y = sy(Number(r[enc.y]) || 0), w = bw * (1 - BARGAP), h = (pT + ih) - y, rx = Math.min(w / 2, h / 2, 6); svg.appendChild(mk('rect', { x: pL + bw * i + bw * BARGAP / 2, y: y, width: w, height: Math.max(h, 0.01), rx: rx, class: scls('mu-chart-bar', r) })); }); } if (kind !== 'scatter') rows.forEach((r, i) => svg.appendChild(mk('text', { x: pL + bw * (i + 0.5), y: H - pB + 15, class: 'mu-chart-xlabel', 'text-anchor': 'middle' }, String(r[enc.x] == null ? '' : r[enc.x])))); doLegend(); }
...
function sourceLayer(parts: EmitParts): string {
  ...
  const __fill = (name, rows) => { const ids = __UUIDS[name] || []; return rows.map((r) => { if (r === null || typeof r !== 'object') return r; const o = { ...r }; for (const f of ids) if (o[f] === null || o[f] === undefined) o[f] = __id(); return o; }); };
